...
def pose_sample(stamp_s: float, right_wrist_body, elbow_angle: float) -> PoseSample3D:
    points = {
        ...
        "left_wrist": body_to_front((-0.25, 0.25, 0.05)),
        "right_wrist": body_to_front(right_wrist_body),
    }
    points["left_elbow"] = bent_elbow(
        points["left_shoulder"], points["left_wrist"], 105.0
    )
    points["right_elbow"] = bent_elbow(
        points["right_shoulder"], points["right_wrist"], elbow_angle
    )
    landmarks = {
...
def event_from_path(path, elbow_angles) -> PunchEvent3D:
    samples = [
        pose_sample(index * 0.1, point, elbow_angles[index])
        for index, point in enumerate(path)
    ]
    return PunchEvent3D(side="right", samples=samples, max_speed=1.0)

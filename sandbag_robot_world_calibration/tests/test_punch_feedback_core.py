...
class ClassificationTests(unittest.TestCase):
    ...
    def test_chamber_drop_then_rise_is_uppercut_and_uses_apex(self):
        start = make_sample(
            0.0,
            {
        ...
        chamber = make_sample(
            0.1,
            {
                "right_elbow": (0.64, 0.65),
                "right_wrist": (0.62, 0.90),
            },
        )
        rising_low = make_sample(
            0.2,
            {
                "right_elbow": (0.66, 0.62),
                "right_wrist": (0.66, 0.74),
            },
        )
        rising_high = make_sample(
            0.3,
            {
                "right_elbow": (0.68, 0.56),
                "right_wrist": (0.74, 0.48),
            },
        )
        apex = make_sample(
            0.4,
            {
                "right_elbow": (0.66, 0.50),
                "right_wrist": (0.82, 0.30),
            },
        )
        recovery = make_sample(
            0.5,
            {
                "right_elbow": (0.67, 0.53),
                "right_wrist": (0.76, 0.42),
            },
        )
        event = PunchEvent(
            "right",
            [start, chamber, rising_low, rising_high, apex, recovery],
            max_speed=4.0,
        )
        classified = classify_punch(
            event,
        ...
        self.assertIs(classified.key_sample, apex)
        self.assertEqual(
            classified.motion_features["uppercut_launch_sample_index"],
            1.0,
        )
        self.assertEqual(
            classified.motion_features["uppercut_impact_sample_index"],
            4.0,
        )
        self.assertEqual(
            classified.motion_features["generic_impact_sample_index"],
            1.0,
        )

    def test_outward_chamber_then_inward_contact_is_hook_and_uses_contact(self):
        start = make_sample(
            0.0,
            {
                "right_elbow": (0.66, 0.50),
        ...
        outward = make_sample(
            0.1,
            {
                "right_elbow": (0.73, 0.44),
                "right_wrist": (0.86, 0.45),
            },
        )
        chamber = make_sample(
            0.2,
            {
                "right_elbow": (0.76, 0.43),
                "right_wrist": (0.94, 0.44),
            },
        )
        inward = make_sample(
            0.3,
            {
                "right_elbow": (0.74, 0.45),
                "right_wrist": (0.82, 0.46),
            },
        )
        contact = make_sample(
            0.4,
            {
                ...
                "right_wrist": (0.68, 0.48),
            },
        )
        recovery = make_sample(
            0.5,
            {
                "right_elbow": (0.68, 0.49),
                "right_wrist": (0.72, 0.48),
            },
        )
        event = PunchEvent(
            "right",
            [start, outward, chamber, inward, contact, recovery],
            max_speed=4.0,
        )
        classified = classify_punch(
            event,
            {
                ...
                "hook_lateral_dominance": 1.10,
                "hook_ordered_max_elbow_angle": 150.0,
                "hook_max_wrist_elbow_height_ratio": 0.40,
            },
        )
        ...
        self.assertIs(classified.key_sample, contact)
        self.assertEqual(
            classified.motion_features["hook_launch_sample_index"],
            2.0,
        )
        self.assertEqual(
            classified.motion_features["hook_impact_sample_index"],
            4.0,
        )
        self.assertEqual(
            classified.motion_features["generic_impact_sample_index"],
    ...
    def test_monotonic_inward_straight_does_not_become_hook(self):
        samples = [
            make_sample(
                0.0,
                {
                    "right_elbow": (0.70, 0.48),
                    "right_wrist": (0.78, 0.48),
                },
            ),
            make_sample(
                0.1,
        ...
        classified = classify_punch(
            PunchEvent("right", samples, max_speed=4.0),
            {
                "trajectory_smoothing_window": 3,
                "hook_min_outward_chamber_ratio": 0.12,
                "hook_min_ordered_inward_ratio": 0.35,
        ...
        self.assertEqual(classified.punch_type, "straight")
        self.assertEqual(
            classified.motion_features["hook_ordered_sweep_pass"],
            0.0,
        )

    def test_single_outward_yolo_spike_does_not_turn_straight_into_hook(self):
        wrist_x_positions = (0.78, 0.86, 0.74, 0.70, 0.66, 0.66)
        samples = [
            make_sample(
                index * 0.1,
                {
                    "right_elbow": (0.70, 0.48),
                    "right_wrist": (wrist_x, 0.48),
                },
            )
            for index, wrist_x in enumerate(wrist_x_positions)
        ]
        classified = classify_punch(
            PunchEvent("right", samples, max_speed=4.0),
            {
                "trajectory_smoothing_window": 3,
                "hook_min_outward_chamber_ratio": 0.06,
                "hook_min_ordered_inward_ratio": 0.55,
                "hook_min_inward_steps": 2,
                "hook_min_inward_consistency": 0.60,
                "hook_min_lateral_ratio": 0.38,
                "hook_lateral_dominance": 1.20,
    ...
    def test_moderate_hook_uses_late_contact_not_outside_chamber(self):
        start = make_sample(
            0.0,
            {
                "right_elbow": (0.66, 0.50),
        ...
        outward = make_sample(
            0.1,
            {
                "right_elbow": (0.70, 0.46),
                "right_wrist": (0.73, 0.46),
            },
        )
        chamber = make_sample(
            0.2,
        ...
        inward_high = make_sample(
            0.3,
            {
                "right_elbow": (0.71, 0.46),
                "right_wrist": (0.71, 0.45),
            },
        )
        inward_low = make_sample(
            0.4,
            {
                "right_elbow": (0.68, 0.48),
                "right_wrist": (0.66, 0.47),
            },
        )
        contact = make_sample(
            0.5,
            {
                "right_elbow": (0.66, 0.49),
                "right_wrist": (0.64, 0.48),
            },
        )
        classified = classify_punch(
            PunchEvent(
                "right",
                [start, outward, chamber, inward_high, inward_low, contact],
                max_speed=4.0,
            ),
            {
                "trajectory_smoothing_window": 3,
                ...
                "hook_min_lateral_ratio": 0.38,
                "hook_lateral_dominance": 1.10,
                "hook_max_elbow_angle": 135.0,
                "hook_ordered_max_elbow_angle": 150.0,
                "hook_max_wrist_elbow_height_ratio": 0.40,
                "straight_min_peak_elbow_angle": 132.0,
                "straight_min_extension_gain_deg": 8.0,
                "straight_max_planar_travel_ratio": 0.32,
                "straight_planar_min_peak_elbow_angle": 132.0,
                "straight_planar_min_extension_gain_deg": 8.0,
            },
        ...
        self.assertIs(classified.key_sample, contact)
        self.assertEqual(
            classified.motion_features["impact_sample_index"],
            5.0,
        )

    def test_small_two_frame_outward_jitter_straight_stays_straight(self):
        wrist_x_positions = (0.78, 0.80, 0.82, 0.75, 0.70, 0.66)
        samples = [
            make_sample(
                index * 0.1,
                {
        ...
        self.assertEqual(classified.punch_type, "straight")
        self.assertIs(classified.key_sample, samples[-1])
        self.assertEqual(
            classified.motion_features["impact_sample_index"],
            5.0,
        )

    def test_wide_straight_drift_is_rejected_by_hook_extension_veto(self):
        wrist_x_positions = (0.70, 0.75, 0.78, 0.74, 0.68, 0.64)
        elbow_points = (
            (0.66, 0.50),
            (0.70, 0.48),
            (0.72, 0.48),
        ...
        samples = [
            ...
            for index, (elbow, wrist_x) in enumerate(
                zip(elbow_points, wrist_x_positions)
            )
        ]
        classified = classify_punch(
            PunchEvent("right", samples, max_speed=4.0),
            {
                ...
                "hook_max_extension_gain_deg": 25.0,
                "hook_lateral_dominance": 1.10,
                "hook_ordered_max_elbow_angle": 180.0,
                "hook_max_wrist_elbow_height_ratio": 0.40,
            },
        )
        ...
        self.assertGreater(
            classified.motion_features["hook_extension_gain_deg"],
            25.0,
        )

    def test_single_vertical_yolo_spike_does_not_curve_straight_into_hook(self):
        wrist_points = (
            (0.78, 0.48),
            (0.75, 0.48),
            (0.72, 0.40),
        ...
        samples = [
            ...
            for index, wrist in enumerate(wrist_points)
        ]
        classified = classify_punch(
            PunchEvent("right", samples, max_speed=4.0),
            {
    ...
    def test_endpoint_vertical_yolo_spikes_do_not_create_hooks(self):
        wrist_x_positions = (0.78, 0.75, 0.72, 0.69, 0.66)
        settings = {
            "trajectory_smoothing_window": 3,
            "hook_min_lateral_ratio": 0.38,
            "hook_lateral_dominance": 1.10,
        ...
        for spike_index in (0, len(wrist_x_positions) - 1):
            with self.subTest(spike_index=spike_index):
                samples = [
                    make_sample(
                        index * 0.1,
                        {
                            ...
                            "right_wrist": (
                                wrist_x,
                                0.40 if index == spike_index else 0.48,
                            ),
                        },
                    )
                    for index, wrist_x in enumerate(wrist_x_positions)
                ...
                self.assertIs(classified.key_sample, samples[-1])
                self.assertEqual(
                    classified.motion_features["hook_priority_pass"],
                    0.0,
                )

    def test_chamberless_curved_hook_uses_arc_contact_frame(self):
        start = make_sample(
            0.0,
            {"right_elbow": (0.62, 0.48), "right_wrist": (0.70, 0.50)},
        )
        curve_high = make_sample(
            0.1,
            {"right_elbow": (0.62, 0.48), "right_wrist": (0.69, 0.40)},
        )
        curve_inside = make_sample(
            0.2,
            {"right_elbow": (0.62, 0.48), "right_wrist": (0.52, 0.34)},
        )
        curve_down = make_sample(
            0.3,
            {"right_elbow": (0.62, 0.48), "right_wrist": (0.54, 0.42)},
        )
        contact = make_sample(
            0.4,
            {"right_elbow": (0.62, 0.48), "right_wrist": (0.50, 0.50)},
        )
        classified = classify_punch(
            PunchEvent(
                "right",
                [start, curve_high, curve_inside, curve_down, contact],
                max_speed=4.0,
            ),
            {
                "trajectory_smoothing_window": 3,
    ...
    def test_ordered_hook_is_symmetric_for_left_and_right(self):
        right_wrist_points = (
            (0.70, 0.48),
            (0.86, 0.45),
            (0.94, 0.44),
        ...
        for side in ("right", "left"):
            with self.subTest(side=side):
                wrist_points = (
                    right_wrist_points
                    if side == "right"
                    else tuple((1.0 - x, y) for x, y in right_wrist_points)
                )
                elbow_points = (
                    right_elbow_points
                    if side == "right"
                    else tuple((1.0 - x, y) for x, y in right_elbow_points)
                )
                samples = [
                    make_sample(
                        index * 0.1,
                    ...
                    for index, (elbow, wrist) in enumerate(
                        zip(elbow_points, wrist_points)
                    )
                ]
                contact = samples[4]
                classified = classify_punch(
                    PunchEvent(side, samples, max_speed=4.0),
                    settings,
                )

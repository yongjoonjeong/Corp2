...
def _median_smooth_1d(
    ...
    *,
    ...
    """Suppress isolated keypoint jitter without shifting the sample indices."""
    if values.size < 3 or window <= 1:
        return values.copy()
    window = min(int(window), values.size if values.size % 2 else values.size - 1)
    window = max(window, 1)
    if window % 2 == 0:
        window -= 1
    if window <= 1:
    ...
    # Edge padding repeats the first/last value, so a spike exactly at an event
    # boundary would otherwise survive the median. Replace only a boundary jump
    # that is much larger than the immediately preceding/following trend. This
    # keeps a genuine monotonic launch/contact while rejecting one-frame YOLO
    # keypoint jumps at capture boundaries.
    if repair_endpoints:
        local_values = values[:window]
        local_floor = max(float(np.ptp(local_values)) * 0.05, 1e-6)
        if abs(float(values[0] - values[1])) > max(
            3.0 * abs(float(values[1] - values[2])),
            local_floor,
        ):
            smoothed[0] = float(np.median(local_values))
        ...
        if abs(float(values[-1] - values[-2])) > max(
            ...
            smoothed[-1] = float(np.median(local_values))
    return smoothed


def _median_smooth_points(points: np.ndarray, window: int) -> np.ndarray:
    if len(points) == 0:
        return points.copy()
...
def _maximum_ordered_drop(values: np.ndarray) -> tuple[int, int, float]:
    """Find the largest drop from an earlier maximum to a later minimum."""
    if values.size < 2:
        return 0, 0, 0.0
    launch_index = 0
    ...
    for index in range(1, values.size):
        ...
        if drop > best_drop:
            ...
            impact_index = index
            best_drop = drop
        # Prefer the latest sample on a smoothed plateau. It represents the end
        # of the chamber, not the first frame that happened to reach it.
...
def classify_punch(
    ...
    trajectory_smoothing_window = max(
        int(settings.get("trajectory_smoothing_window", 3)),
        1,
    )
    if trajectory_smoothing_window % 2 == 0:
        trajectory_smoothing_window += 1
    smoothed_torso_relative_points = _median_smooth_points(
        torso_relative_points,
        trajectory_smoothing_window,
    )
    smoothed_all_elbow_angles = _median_smooth_1d(
        all_elbow_angles,
        trajectory_smoothing_window,
    )
    # Very short synthetic/low-FPS events do not contain enough neighbors for
    # temporal smoothing. Normal webcam events do, and smoothing there prevents
    # a single YOLO wrist/elbow spike from creating a fake curved Hook.
    use_smoothed_trajectory = len(event.samples) >= max(
        trajectory_smoothing_window + 2,
        5,
    )
    classification_torso_relative_points = (
        smoothed_torso_relative_points
        if use_smoothed_trajectory
        else torso_relative_points
    )
    classification_all_elbow_angles = (
        smoothed_all_elbow_angles
        if use_smoothed_trajectory
        else all_elbow_angles
    )
    trajectory_deltas = (
        classification_torso_relative_points
        - classification_torso_relative_points[0]
    )
    ...
    all_extension_gains = np.maximum(
        classification_all_elbow_angles - classification_all_elbow_angles[0],
        0.0,
    )

    # An uppercut commonly starts with a downward chamber. That preparation can
    # be farther from guard (and straighter) than the actual upward strike, so a
    # generic reach/extension peak can incorrectly stop at the chamber. Find an
    # ordered low-to-high segment over the complete event before trimming any
    # recovery frames. The smoothed path chooses the segment and raw points then
    # refine the actual launch/apex so the displayed impact stays on the fist.
    uppercut_smoothing_window = max(
        int(settings.get("uppercut_smoothing_window", 3)),
        1,
    )
    if uppercut_smoothing_window % 2 == 0:
        uppercut_smoothing_window += 1
    smoothed_vertical_positions = _median_smooth_1d(
        torso_relative_points[:, 1],
        uppercut_smoothing_window,
        repair_endpoints=True,
    )
    (
        smoothed_uppercut_launch_index,
        smoothed_uppercut_apex_index,
        _,
    ) = _maximum_ordered_drop(smoothed_vertical_positions)
    smoothing_radius = uppercut_smoothing_window // 2
    uppercut_launch_index = smoothed_uppercut_launch_index
    uppercut_apex_index = smoothed_uppercut_apex_index
    if smoothed_uppercut_apex_index > smoothed_uppercut_launch_index:
        launch_search_start = max(
            0,
            smoothed_uppercut_launch_index - smoothing_radius,
        )
        launch_search_stop = min(
            smoothed_uppercut_apex_index,
            smoothed_uppercut_launch_index + smoothing_radius,
        )
        uppercut_launch_index = launch_search_start + int(
            np.argmax(
                torso_relative_points[
                    launch_search_start : launch_search_stop + 1,
                    1,
                ]
            )
        )

        apex_search_start = max(
            uppercut_launch_index + 1,
            smoothed_uppercut_apex_index - smoothing_radius,
        )
        apex_search_stop = min(
            len(event.samples) - 1,
            smoothed_uppercut_apex_index + smoothing_radius,
        )
        if apex_search_start <= apex_search_stop:
            uppercut_apex_index = apex_search_start + int(
                np.argmin(
                    torso_relative_points[
                        apex_search_start : apex_search_stop + 1,
                        1,
                    ]
                )
            )
    ...
    uppercut_rise = max(
        float(
            torso_relative_points[uppercut_launch_index, 1]
            - torso_relative_points[uppercut_apex_index, 1]
        ),
        0.0,
    )
    uppercut_impact_peak_fraction = float(
        settings.get("uppercut_impact_peak_fraction", 0.95)
    )
    uppercut_impact_peak_fraction = float(
        np.clip(uppercut_impact_peak_fraction, 0.50, 1.0)
    )
    uppercut_impact_index = uppercut_apex_index
    if uppercut_rise > 1e-9 and uppercut_apex_index > uppercut_launch_index:
        near_apex_height = (
            torso_relative_points[uppercut_launch_index, 1]
            - uppercut_rise * uppercut_impact_peak_fraction
        )
        near_apex_candidates = np.flatnonzero(
            torso_relative_points[
        ...
        if near_apex_candidates.size:
            uppercut_impact_index = (
                uppercut_launch_index + int(near_apex_candidates[0])
            )

    uppercut_path = torso_relative_points[
        uppercut_launch_index : uppercut_impact_index + 1
    ]
    uppercut_lateral = (
        float(
            np.max(
                np.abs(uppercut_path[:, 0] - uppercut_path[0, 0])
            )
        )
        if len(uppercut_path)
        else 0.0
    )
    all_wrist_above_elbow_values = (
        (all_elbow_points[:, 1] - all_wrist_points[:, 1])
        * vertical_aspect
        / sample_scales
    )

    # Hook needs its own impact phase as well. A real hook can first extend to
    # an outside chamber, then strike inward. The generic reach/extension peak
    # stops at that chamber and discards the actual strike as recovery. Locate
    # the largest ordered outside-to-inside sweep over the complete event. The
    # smoothed coordinates choose a stable phase, while key_sample still points
    # to the original YOLO frame at the selected contact index.
    hook_start_offset = float(
        np.median(
            smoothed_torso_relative_points[
                : min(3, len(smoothed_torso_relative_points)),
                0,
            ]
        )
    )
    hook_outward_sign = 1.0 if hook_start_offset >= 0.0 else -1.0
    smoothed_hook_outward_positions = (
        hook_outward_sign * smoothed_torso_relative_points[:, 0]
    )
    raw_hook_outward_positions = hook_outward_sign * torso_relative_points[:, 0]
    (
        hook_launch_index,
        hook_terminal_index,
        _,
    ) = _maximum_ordered_drop(smoothed_hook_outward_positions)
    hook_inward_sweep = max(
        float(
            smoothed_hook_outward_positions[hook_launch_index]
            - smoothed_hook_outward_positions[hook_terminal_index]
        ),
        0.0,
    )
    hook_outward_excursion = max(
        float(
            smoothed_hook_outward_positions[hook_launch_index]
            - smoothed_hook_outward_positions[0]
        ),
        0.0,
    )
    hook_impact_peak_fraction = float(
        settings.get("hook_impact_peak_fraction", 0.92)
    )
    hook_impact_peak_fraction = float(
        np.clip(hook_impact_peak_fraction, 0.50, 1.0)
    )
    ...
    if hook_inward_sweep > 1e-9 and hook_terminal_index > hook_launch_index:
        near_contact_position = (
            smoothed_hook_outward_positions[hook_launch_index]
            - hook_inward_sweep * hook_impact_peak_fraction
        )
        hook_contact_candidates = np.flatnonzero(
            smoothed_hook_outward_positions[
                hook_launch_index : hook_terminal_index + 1
            ]
            <= near_contact_position
        )
        if hook_contact_candidates.size:
            hook_impact_index = (
                hook_launch_index + int(hook_contact_candidates[0])
            )

    hook_outward_segment = smoothed_hook_outward_positions[
        hook_launch_index : hook_impact_index + 1
    ]
    hook_inward_step_values = -np.diff(hook_outward_segment)
    hook_inward_steps = int(np.count_nonzero(hook_inward_step_values > 1e-4))
    hook_sweep_variation = float(np.sum(np.abs(hook_inward_step_values)))
    hook_selected_inward_sweep = max(
        float(
            smoothed_hook_outward_positions[hook_launch_index]
            - smoothed_hook_outward_positions[hook_impact_index]
        ),
        0.0,
    )
    hook_inward_consistency = (
        float(
            np.clip(
                hook_selected_inward_sweep / hook_sweep_variation,
                0.0,
                1.0,
            )
        )
        if hook_sweep_variation > 1e-9
        else 0.0
    )
    hook_outward_step_values = np.diff(
        raw_hook_outward_positions[: hook_launch_index + 1]
    )
    hook_raw_outward_excursion = max(
        float(
            raw_hook_outward_positions[hook_launch_index]
            - raw_hook_outward_positions[0]
        ),
        0.0,
    )
    hook_outward_steps = int(
        np.count_nonzero(hook_outward_step_values > 1e-4)
    )
    hook_outward_variation = float(np.sum(np.abs(hook_outward_step_values)))
    hook_outward_consistency = (
        float(
            np.clip(
                hook_raw_outward_excursion / hook_outward_variation,
                0.0,
                1.0,
            )
        )
    ...
    hook_ordered_path = smoothed_torso_relative_points[
        hook_launch_index : hook_impact_index + 1
    ]
    hook_ordered_vertical_travel = (
        float(
            np.max(
                np.abs(hook_ordered_path[:, 1] - hook_ordered_path[0, 1])
            )
        )
        if len(hook_ordered_path)
        else 0.0
    )
    (
        hook_ordered_path_length,
        hook_ordered_direct_travel,
        hook_ordered_path_linearity,
        hook_ordered_path_curvature,
    ) = _trajectory_geometry(hook_ordered_path)
    hook_ordered_impact_elbow_angle = float(
        smoothed_all_elbow_angles[hook_impact_index]
    )
    hook_ordered_extension_gain = max(
        float(
            np.max(
                smoothed_all_elbow_angles[
                    hook_launch_index : hook_impact_index + 1
            ...
            - smoothed_all_elbow_angles[hook_launch_index]
        ),
        0.0,
    )
    hook_outward_vector = (
        smoothed_torso_relative_points[hook_launch_index]
        - smoothed_torso_relative_points[0]
    )
    hook_inward_vector = (
        smoothed_torso_relative_points[hook_impact_index]
        - smoothed_torso_relative_points[hook_launch_index]
    )
    hook_outward_norm = float(np.linalg.norm(hook_outward_vector))
    hook_inward_norm = float(np.linalg.norm(hook_inward_vector))
    hook_turn_angle = 0.0
    if hook_outward_norm > 1e-9 and hook_inward_norm > 1e-9:
        hook_turn_angle = float(
            np.degrees(
                np.arccos(
                    np.clip(
                        np.dot(hook_outward_vector, hook_inward_vector)
                        / (hook_outward_norm * hook_inward_norm),
                        -1.0,
                        1.0,
                    )
                )
    ...
    hook_ordered_wrist_elbow_height = float(
        all_wrist_above_elbow_values[hook_impact_index]
    )
    full_event_lateral_sweep = float(
        np.ptp(smoothed_torso_relative_points[:, 0])
    )
    ...
    elbow_angles = classification_all_elbow_angles[outbound]
    trajectory_points = classification_torso_relative_points[outbound]
    ...
    hook_min_outward_chamber = float(
        settings.get("hook_min_outward_chamber_ratio", 0.12)
    )
    hook_min_ordered_inward = float(
        settings.get("hook_min_ordered_inward_ratio", 0.35)
    )
    ...
    hook_ordered_max_elbow = float(
        settings.get("hook_ordered_max_elbow_angle", 150.0)
    )
    hook_ordered_max_extension_gain = float(
        settings.get("hook_max_extension_gain_deg", 25.0)
    )
    hook_min_turn_angle = float(
        settings.get("hook_min_turn_angle_deg", 70.0)
    )
    hook_arc_contact_min_inward = float(
    ...
    # A monotonic front-camera straight naturally moves from an outside guard
    # toward image center. Inward displacement alone is therefore not Hook
    # evidence; it must either form a curved path or contain a real outside
    # chamber followed by a sustained inward strike.
    hook_arc_evidence = hook_path_curved
    hook_direction_pass = (
    ...
    hook_ordered_sweep_detected = (
        hook_outward_excursion >= hook_min_outward_chamber
        and hook_outward_steps >= hook_min_outward_steps
        and hook_outward_consistency >= hook_min_outward_consistency
        and hook_inward_sweep >= hook_min_ordered_inward
        and hook_inward_steps >= hook_min_inward_steps
        and hook_inward_consistency >= hook_min_inward_consistency
        and hook_turn_angle >= hook_min_turn_angle
        and hook_inward_sweep
        >= hook_ordered_vertical_travel * hook_lateral_dominance
        and hook_ordered_impact_elbow_angle <= hook_ordered_max_elbow
        and hook_ordered_extension_gain <= hook_ordered_max_extension_gain
        and abs(hook_ordered_wrist_elbow_height)
        <= hook_max_wrist_elbow_height
    )
    hook_has_late_contact = (
        hook_terminal_index > hook_launch_index
        and hook_inward_sweep >= hook_arc_contact_min_inward
    )
    hook_class_impact_index = (
        hook_impact_index if hook_has_late_contact else impact_index
    )
    uppercut_competing_lateral = max(
        uppercut_lateral,
        full_event_lateral_sweep,
    )
    uppercut_direction_evidence = (
        uppercut_rise >= uppercut_min_up
        and uppercut_rise
        >= uppercut_competing_lateral * uppercut_vertical_dominance
    )
    uppercut_detected = (
        uppercut_direction_evidence
        and all_elbow_angles[uppercut_impact_index] <= uppercut_max_elbow
        and all_wrist_above_elbow_values[uppercut_impact_index]
        >= uppercut_min_wrist_above
    )
    ...
    straight_low_planar_detected = (
        ...
        and not uppercut_direction_evidence
    )
    ...
    straight_extension_priority_detected = strong_2d_straight
    ...
    uppercut_fallback = (
        not straight_detected
        and uppercut_rise >= uppercut_min_up * 0.70
        and uppercut_rise > uppercut_lateral
        and all_wrist_above_elbow_values[uppercut_impact_index] > 0.0
    )
    ...
    if hook_ordered_sweep_detected:
        punch_type = "hook"
        key_index = hook_class_impact_index
        classification_reason = "hook_outside_to_inside"
        confidence = 0.65 + min(
            0.30,
            max(
                hook_inward_sweep - hook_min_ordered_inward,
                hook_inward_consistency - hook_min_inward_consistency,
            ),
        )
    elif hook_priority_detected:
        punch_type = "hook"
        key_index = hook_class_impact_index
        ...
        punch_type = "uppercut"
        key_index = uppercut_impact_index
        classification_reason = "uppercut_vertical_dominant"
        confidence = 0.60 + min(0.35, uppercut_rise - uppercut_min_up)
    ...
    selected_upward = uppercut_rise if punch_type == "uppercut" else upward
    selected_lateral = uppercut_lateral if punch_type == "uppercut" else lateral
    if punch_type == "hook" and hook_has_late_contact:
        selected_lateral = hook_inward_sweep
    selected_direction_angle = float(
        np.degrees(
            np.arctan2(
                max(selected_upward, 0.0),
                max(selected_lateral, 1e-6),
            )
        )
    )
    motion_features = {
        "upward_ratio": selected_upward,
        "lateral_ratio": selected_lateral,
        ...
        "inward_ratio": inward,
        "direction_angle_deg": selected_direction_angle,
        "wrist_above_elbow_ratio": float(
            all_wrist_above_elbow_values[key_index]
        ),
        "extension_gain_deg": extension_gain,
        "max_elbow_angle_deg": max_elbow_angle,
        "impact_elbow_angle_deg": float(
            classification_all_elbow_angles[key_index]
        ),
        ...
        "straight_extension_priority_pass": float(
            straight_extension_priority_detected
        ),
        "uppercut_direction_pass": float(uppercut_direction_evidence),
        "uppercut_strict_pass": float(uppercut_detected),
        "uppercut_rise_ratio": uppercut_rise,
        "uppercut_lateral_ratio": uppercut_lateral,
        ...
        "hook_priority_pass": float(
            hook_priority_detected or hook_ordered_sweep_detected
        ),
        "hook_ordered_sweep_pass": float(hook_ordered_sweep_detected),
        ...
        "hook_raw_outward_excursion_ratio": hook_raw_outward_excursion,
        ...
        "hook_extension_gain_deg": hook_ordered_extension_gain,
        "hook_late_contact_pass": float(hook_has_late_contact),
        "hook_ordered_vertical_travel_ratio": hook_ordered_vertical_travel,
        "hook_ordered_path_length_ratio": hook_ordered_path_length,
        "hook_ordered_direct_travel_ratio": hook_ordered_direct_travel,
        "hook_ordered_path_linearity": hook_ordered_path_linearity,
        "hook_ordered_path_curvature_ratio": hook_ordered_path_curvature,
        "hook_ordered_impact_elbow_angle_deg": (
            hook_ordered_impact_elbow_angle
        ),
        "hook_strict_pass": float(hook_detected),
        "hook_fallback_pass": float(hook_fallback),
        "impact_sample_index": float(key_index),
        "generic_impact_sample_index": float(impact_index),
        "event_sample_count": float(len(event.samples)),
        "recovery_frames_ignored": float(len(event.samples) - key_index - 1),

...
# MediaPipe Pose landmark indices are kept as plain integers so YOLO mode does
# not need to import MediaPipe at module startup.
LANDMARK_INDEX = {
    "nose": 0,
    "left_shoulder": 11,
    "right_shoulder": 12,
...
class WebcamPunchFeedbackNode(Node):
    def __init__(self) -> None:
        ...
        default_pose_model = (
            Path(__file__).resolve().parent / "models" / "yolo11n-pose.pt"
        )
        ...
        self.declare_parameter("pose_backend", "mediapipe")
        ...
        self.pose_backend_name = str(
            self.get_parameter("pose_backend").value
        ).strip().lower()
        ...
        if self.pose_backend_name not in ("mediapipe", "yolo11n"):
            raise ValueError(
                "pose_backend must be either 'mediapipe' or 'yolo11n'"
            )
        ...
        self.pose = None
        self.yolo_pose_backend: YoloPoseBackend | None = None
        self.yolo_pose_selector: SingleViewPoseSelector | None = None
        if self.pose_backend_name == "yolo11n":
            self.yolo_pose_backend = YoloPoseBackend(
                self.pose_model,
            ...
            self.yolo_pose_selector = SingleViewPoseSelector(
            ...
            try:
                ...
                raise RuntimeError(
                    "MediaPipe backend is unavailable. Run ./setup_3d.sh or "
                    "select pose_backend:=yolo11n."
                ) from error
        ...
        self.window_name = (
            "Sandbag Punch Feedback MVP - YOLO11n-Pose"
            if self.pose_backend_name == "yolo11n"
            else "Sandbag Punch Feedback MVP - MediaPipe"
        )
        if self.display:
        ...
        self.get_logger().info(
            f"Webcam MVP ready: camera={self.camera_index}, "
            f"pose_backend={self.pose_backend_name}, reference={reference_path}"
        )
        if self.yolo_pose_backend is not None:
            self.get_logger().info(
                "YOLO11n-Pose 2D test: "
                f"device={self.yolo_pose_backend.device}, "
                f"imgsz={self.pose_image_size}; R resets the boxer target"
            )
            if self.yolo_pose_backend.device_fallback_reason is not None:
                self.get_logger().warning(
                    "YOLO auto device fallback to CPU: "
                    + self.yolo_pose_backend.device_fallback_reason
                )
            self.get_logger().warning(
                "YOLO Pose has no landmark z; the unchanged 2D classifier "
                "uses its planar straight rules."
            )
        self.get_logger().info(
    ...
    def on_frame(self) -> None:
        ...
        if self.yolo_pose_backend is not None:
            candidates = self.yolo_pose_backend.infer({"webcam": frame})["webcam"]
            self.last_people_count = len(candidates)
            detection = self.yolo_pose_selector.select(
                candidates,
                frame.shape[1],
                frame.shape[0],
            )
            sample = (
                detection.to_pose_sample(time.monotonic(), frame)
            ...
            self.last_people_count = int(sample is not None)
        ...
        if self.display:
            ...
            if (
                pressed_key in (ord("r"), ord("R"))
                and self.yolo_pose_selector is not None
            ):
                self.yolo_pose_selector.reset()
                self.detector.reset()
    ...
    def draw_preview_status(self, frame) -> None:
        ...
        target_state = (
            self.yolo_pose_selector.state
            if self.yolo_pose_selector is not None
            else "SINGLE"
        )
        backend_label = (
            f"YOLO P{self.last_people_count} {target_state}"
            if self.pose_backend_name == "yolo11n"
            else "MEDIAPIPE"
        )
        cv2.putText(
            frame,
            f"STATE: {self.detector.state} | {backend_label}",
            (12, 26),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.58,
            (0, 255, 255),
    ...
    def handle_punch(self, event) -> None:
        ...
        payload = {
            ...
            "pose_backend": self.pose_backend_name,
        ...
        self.get_logger().info(
            ...
            f"lateral={motion.get('lateral_ratio', 0.0):.2f} "
            f"inward={motion.get('inward_ratio', 0.0):.2f} "
            f"hook_out={motion.get('hook_outward_excursion_ratio', 0.0):.2f}/"
            f"{motion.get('hook_outward_steps', 0.0):.0f} "
            f"hook_in={motion.get('hook_inward_sweep_ratio', 0.0):.2f}/"
            f"{motion.get('hook_inward_steps', 0.0):.0f} "
            f"turn={motion.get('hook_turn_angle_deg', 0.0):.0f} "
            f"hook_ext={motion.get('hook_extension_gain_deg', 0.0):.0f} "
            f"impact={motion.get('impact_sample_index', 0.0):.0f}/"
            f"{motion.get('generic_impact_sample_index', 0.0):.0f} "
    ...
    def destroy_node(self) -> bool:
        ...
        if hasattr(self, "pose") and self.pose is not None:
            self.pose.close()
        if (
            hasattr(self, "yolo_pose_backend")
            and self.yolo_pose_backend is not None
        ):
            self.yolo_pose_backend.close()

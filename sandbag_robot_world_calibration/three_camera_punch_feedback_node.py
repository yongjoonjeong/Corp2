...
from webcam_punch_feedback_node import (
    LANDMARK_INDEX,
    PUNCH_COLOR_BGR,
    draw_guard_gauge,
    draw_pose,
...
def landmark_observations(
    ...
    for name, index in LANDMARK_INDEX.items():
        ...
        x = float(landmark.x)
        y = float(landmark.y)
        if confidence < min_visibility or not (-0.05 <= x <= 1.05 and -0.05 <= y <= 1.05):
            continue
...
class ThreeCameraPunchFeedbackNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("min_landmark_visibility", 0.20)
        ...
        from webcam_punch_feedback_node import mp_pose
        ...
        self.poses = {
            camera: mp_pose.Pose(
                **pose_options,
                enable_segmentation=(camera == "front"),
                smooth_segmentation=(camera == "front"),
            )
    ...
    def on_frame(self) -> None:
        ...
        pose_results = {
            camera: self.poses[camera].process(
                cv2.cvtColor(timed_frames[camera].image, cv2.COLOR_BGR2RGB)
            )
            for camera in ("left", "front", "right")
        }
        ...
        if (
            ...
            person_points_base, person_colors = colored_person_point_cloud_base(
                ...
                getattr(pose_results["front"], "segmentation_mask", None),
                self.color_intrinsics,
                self.depth_scale_m,
        ...
        self.last_pose_detected = {
            camera: bool(pose_results[camera].pose_landmarks)
            for camera in CAMERA_DISPLAY_ORDER
        }
        front_pose = to_pose_sample(
            pose_results["front"],
            timed_frames["front"].stamp_s,
            timed_frames["front"].image,
        )
        self.publish_detected_wrist_coordinates(
            timed_frames, depth_frame, pose_results
        )
        ...
        if self.display:
            ...
            for camera in ("left", "right"):
                display_samples[camera] = to_pose_sample(
                    pose_results[camera],
        ...
        message.data = json.dumps(
            {
                ...
                "fist_proxy": "mediapipe_pose_wrist",
    ...
    def log_runtime_diagnostics(self) -> None:
        ...
        self.get_logger().info(
            ...
            f"mp=L{int(self.last_pose_detected['left'])}"
    ...
    def build_camera_label_strip(self) -> np.ndarray:
        ...
        for index, camera in enumerate(CAMERA_DISPLAY_ORDER):
            ...
            pose_label = "MP OK" if pose_detected else "MP NO POSE"

...
## YOLO11n-Pose 단일 웹캠 2D 테스트

기존 2D 펀치 검출·분류·점수·JPEG·ROS 토픽을 그대로 사용하고 관절 검출만
MediaPipe에서 YOLO11n-Pose로 바꿔 비교할 수 있습니다. 기존 MediaPipe 실행
경로는 삭제하지 않았습니다.
...
다른 웹캠 인덱스나 CPU를 명시하려면 wrapper가 이미 `--ros-args`를 넣으므로
`-p`부터 전달합니다.
...
./run_yolo_2d_test.sh \
  -p camera_index:=2 \
  -p pose_device:=cpu \
  -p pose_image_size:=640 \
  -p pose_detection_confidence:=0.25
```

기본 `pose_device:=auto`는 CUDA 사용 가능 시 GPU 0, 아니면 CPU를 선택합니다.
화면의 `YOLO P2 LOCKED`는 두 명을 검출했고 그중 한 복서를 잠갔다는 뜻입니다.
대상을 잘못 선택하면 `R`, 종료는 `Q` 또는 `Esc`를 누릅니다.
...
YOLO Pose는 MediaPipe의 상대 `z`에 해당하는 값을 제공하지 않으므로 이 2D
테스트에서는 `z=0`으로 어댑트합니다. 기존 분류 코드는 변경하지 않았으며,
스트레이트는 기존의 화면상 직선성·이동량·팔꿈치 신전 조건으로 판정합니다.
3D 런타임은 삼각측량과 RealSense Depth를 사용하므로 이 제한이 없습니다.

전용 `config/temporary_form_reference_yolo11_2d.yaml`은 기존 2D 값과 동일하고,
서로 다른 confidence 의미를 맞추기 위해 `min_visibility`만 YOLO 검증값 `0.35`로
설정했습니다. 기존 MediaPipe용 YAML에는 영향을 주지 않습니다.
...
포인트클라우드, 입체형 상체 아바타와 손목 BASE 좌표가 나옵니다. 세 카메라의
관절은 YOLO11n-Pose 한 번의 배치 추론으로 검출합니다. 여러 사람이 보이면
캘리브레이션 재투영 오차와 직전 위치를 사용해 세 화면에서 같은 복서를 선택합니다.
정면 사람 포인트클라우드의 배경은 선택된 YOLO 박스와 RealSense 몸통 Depth
대역으로 제거합니다. 이 런타임은 로봇 이동 명령을 보내지 않습니다.
...
./run_ros_3d_mvp.sh --ros-args \
  ...
  -p point_cloud_depth_band_mm:=300.0
```

`point_cloud_stride`가 작을수록 표면이 촘촘하지만 처리량이 증가합니다. 기본값 `4`는
640×480 입력의 약 1/16 픽셀을 사용합니다.

YOLO 입력 크기, 키포인트 confidence, 실행 장치를 바꾸려면:

```bash
./run_ros_3d_mvp.sh --ros-args \
  -p pose_image_size:=640 \
  -p min_landmark_visibility:=0.35 \
  -p pose_device:=auto
```

`pose_device:=auto`는 CUDA를 사용할 수 있으면 `cuda:0`, 아니면 CPU를 자동으로
사용합니다. 대상 복서를 잘못 잠갔을 때는 영상 창에서 `R`을 눌러 다시 선택합니다.
